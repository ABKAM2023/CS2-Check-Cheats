...
namespace CheckCheatsPlugin
{
    public class CheckCheatsPlugin : BasePlugin
    {
        ...
        private async Task SendDiscordWebhookNotification(string webhookUrl, string playerName, string adminName, string playerSteamId64, string adminSteamId64)
        {
            var embed = new
            {
                title = $"Admin {adminName} summoned player {playerName} for inspection",
                color = 16777215, 
                fields = new[]
                {
                    new
        ...
        public class PluginConfig
        {
            ...
            public string CountdownMessageFormat { get; set; } = "<font color='red' class='fontSize-l'>You are called for inspection. Provide your Discord. Remaining time: {remainingTime} seconds. Use !contact your_discord.</font>";
            public string ErrorMessage { get; set; } = "[{Red}ADMIN{White}] Please specify your Discord. Use: {Green}!contact your_discord";
            public string AdminMessageFormat { get; set; } = "[{Red}ADMIN{White}] Player {Yellow}{PlayerName} {White}provided their Discord: {Green}{DiscordContact}";
            public string SuccessMessage { get; set; } = "<font color='green' class='fontSize-l'>You have successfully passed the inspection.</font>";
